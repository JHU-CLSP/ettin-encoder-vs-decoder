...
def process_shard(shard_path, output_path, tokenizer_name, batch_size):
    ...
    with MDSWriter(out=output_path, columns=PROLONG_COLS, compression='zstd') as mds_writer:
    ...
    for root, _, files in os.walk(main_folder):
        for file in files:
            if file.endswith('index.json'):
                # check if it is unique by looking in the output dir for it and the index.json
                if (not os.path.exists(os.path.join(output_folder, os.path.splitext(os.path.relpath(root, main_folder))[0])) or not os.path.exists(os.path.join(output_folder, os.path.splitext(os.path.relpath(root, main_folder))[0], 'index.json'))) and os.path.exists(os.path.join(root, 'shard.000000.mds')):
